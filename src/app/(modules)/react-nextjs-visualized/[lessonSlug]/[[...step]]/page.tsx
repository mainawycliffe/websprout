...
import ReactNextjsVisualizedLessonClient from "../client";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const lesson = getLesson("react-nextjs-visualized", lessonSlug);
  if (!lesson) return {};
  const mod = getModule("react-nextjs-visualized")!;
  return { title: `${lesson.title} – ${mod.title}`, description: lesson.description };
}

export default async function ReactNextjsVisualizedLessonPage({ params }: Props) {
  const { lessonSlug, step } = await params;
  const lesson = getLesson("react-nextjs-visualized", lessonSlug);
  if (!lesson) notFound();
  const initialStep = step?.[0] ? parseInt(step[0], 10) || 0 : 0;
  return <ReactNextjsVisualizedLessonClient lesson={lesson} lessonSlug={lessonSlug} initialStep={initialStep} />;
}
