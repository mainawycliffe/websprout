...
import DataFetchingLessonClient from "../client";
...
export async function generateStaticParams() {
  const mod = getModule("data-fetching");
  if (!mod) return [];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const lesson = getLesson("data-fetching", lessonSlug);
  if (!lesson) return {};
  const mod = getModule("data-fetching")!;
  return { title: `${lesson.title} – ${mod.title}`, description: lesson.description };
}

export default async function DataFetchingLessonPage({ params }: Props) {
  const { lessonSlug, step } = await params;
  const lesson = getLesson("data-fetching", lessonSlug);
  if (!lesson) notFound();
  const initialStep = step?.[0] ? parseInt(step[0], 10) || 0 : 0;
  return <DataFetchingLessonClient lesson={lesson} lessonSlug={lessonSlug} initialStep={initialStep} />;
}
