...
import JsPracticeTsLessonClient from "../client";
...
export async function generateStaticParams() {
  const mod = getModule("js-practice-ts");
  if (!mod) return [];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const lesson = getLesson("js-practice-ts", lessonSlug);
  if (!lesson) return {};
  const mod = getModule("js-practice-ts")!;
  return { title: `${lesson.title} – ${mod.title}`, description: lesson.description };
}

export default async function JsPracticeTsLessonPage({ params }: Props) {
  const { lessonSlug, step } = await params;
  const lesson = getLesson("js-practice-ts", lessonSlug);
  if (!lesson) notFound();
  const initialStep = step?.[0] ? parseInt(step[0], 10) || 0 : 0;
  return <JsPracticeTsLessonClient lesson={lesson} lessonSlug={lessonSlug} initialStep={initialStep} />;
}
