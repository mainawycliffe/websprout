...
export default function InfoBox({ box }: InfoBoxProps) {
  ...
  return (
    <motion.div
      ...
      <div
        className='text-sm text-text leading-relaxed [&>p]:my-0 [&>ul]:list-disc [&>ul]:list-inside [&>ul]:flex [&>ul]:flex-col [&>ul]:gap-1 [&>ol]:list-decimal [&>ol]:list-inside [&>ol]:flex [&>ol]:flex-col [&>ol]:gap-1 [&_code]:bg-black/10 [&_code]:px-1 [&_code]:rounded [&_code]:text-xs [&_code]:font-mono'
        dangerouslySetInnerHTML={{ __html: renderLessonHtml(box.body) }}
      />
    </motion.div>
  );
}
